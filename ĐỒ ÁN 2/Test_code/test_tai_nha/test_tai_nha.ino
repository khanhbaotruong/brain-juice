...
const char* ssid = "Thu Ba";
const char* password = "annhien2016";
const char* mqtt_server = "192.168.1.4";

// const char* ssid = "nth";
// const char* password = "nama51309";

// const char* ssid = "CHICO CAFE 6";
// const char* password = "66666666";

// const char* ssid = "Redmi 10";
// const char* password = "0374152855";
...
uint8_t muccanhbao_soil_nguongduoi = 80;
...
float setpoint = 31.0f;  // nhiệt độ tốt - đặt cho cây phát triển.
...
void IRAM_ATTR ngat() {
  if (digitalRead(ngatdiem0) == 0) {
    ...
    while (digitalRead(ngatdiem0) == 0) {
      ...
      if (digitalRead(den) == 0) { // trong hàm ngắt thì ko nên để 1 biến mà biến đó được đọc từ 1 thư viện hoặc một ngắt khác
        break;
...
void loop() {
  ...
  if (t < 30 && PT100_Temperature < 30 && tuoicay == 0) {
    PID_Nhietdo = 1;
    batquat_1 = 0;
    batquat_2 = 0;
    batquat_3 = 0;
  ...
  if ((t > 32 && PT100_Temperature > 32) && tuoicay == 0) {
    PID_Nhietdo = 0;
    batquat_1 = 0;
    batquat_2 = 1;
    batquat_3 = 0;
  ...
  // trường hợp ngoại lệ
  if ((t < 30 && PT100_Temperature > 32) && tuoicay == 0)  // vì một lý do nào đó nhiệt độ môi trường khá thấp, nhưng nhiệt độ trong nhà kính > 28.5
  {
    PID_Nhietdo = 0;
    batquat_1 = 0;
    batquat_2 = 0;
  ...
  if (batquat_2 == 1) {
    ...
    if (phun_suong == 1 && PT100_Temperature > 32) {
      digitalWrite(quat, 0);
    ...
    if ((millis() - now_phunsuong > 3000) && bat_quat == 1) {
      ledcWrite(suongChannel, 0);
      now_quat = millis();
      digitalWrite(quat, 1);
      bat_quat = 0;
    ...
    if (millis() - now_quat > 7000 && bat_quat == 0)  // bật quạt 7 giây sau khi phun sương 3 giây
    {
      phun_suong = 1;
    }
  }

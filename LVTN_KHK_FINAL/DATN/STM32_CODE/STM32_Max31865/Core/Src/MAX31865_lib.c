...
#define MAX31865_PT100_R0 (double)100.0 //Сопротивление датчика PT100, при 0 °С //Khai báo biến R0 = 100 tại 0 độ
#define MAX31865_Rref (double)418.5 //Сопротивление референсного резистора, подключенного к MAX31865 điện trở Rref trên board
/*--------------Характеристики датчика типа PT100 и референсный резистор, подключенный к MAX31865------------*/

/*-----------Коэффициенты из ГОСТ 6651-2009 для датчика типа PT100(Платиновые ТС И ЧЭ, 0.00385°С^-1)---------*/
#define MAX31865_A (double)0.0039083 //Khai báo các biến cần thiết từ trang 10 datasheet
...
/*-------------------------------------------Для работы по spi-----------------------------------------------*/
#define cs_set() CS_GPIO_Port-> BSRR = (uint32_t) CS_Pin << 16u; //CS_притягиваем к земле // Đặt mức 0 tương đương với tín hiệu set CS
